...
class Vaccine:
    ...
    def select_vaccines():
        ...
        try:
            ...
            for row in cursor:
                print(row["Name"] + ": " + str(row["Doses"]) + " doses")
        except pymssql.Error:
            raise
        finally:
            cm.close_connection()
    ...
    def ret_doses(name):
        ...
        retrieve_vaccine = "SELECT Name, Doses FROM Vaccines WHERE Name = %s"
        try:
            cursor.execute(retrieve_vaccine, name)
            for row in cursor:
                return Vaccine(row["Name"], row["Doses"])
        except pymssql.Error:
            raise
        finally:
            cm.close_connection()
